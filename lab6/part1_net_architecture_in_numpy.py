...
def zad1_single_neuron(student_id):
    ...
    class SingleNeuron:
        ...
        def forward(self, x_data):
            ...
            """
            # TODO (0.5 point)
            logits = x_data @ self.W + self.b  # shape: [n_samples, 1]
...
def zad2_two_layer_net(student_id):
    ...
    class DenseLayer:
        ...
        def forward(self, x_data):
            # TODO
            logits = x_data @ self.W + self.b  # shape: [n_samples, n_out]
            return self.f_act(logits)
    ...
    class SimpleTwoLayerNetwork:
        ...
        def forward(self, x_data):
            ...
            y_pred = self.output_layer.forward(h)
            return y_pred
    ...
    model.hidden_layer.W[:, 0] = [10, -10]
    model.hidden_layer.W[:, 1] = [-10, 10]
    model.hidden_layer.b[:] = [-5, -5]

    model.output_layer.W[:, 0] = [10, 10]
    model.output_layer.b[:] = [-10]
